...
class a2c_agent():
    ...
    def fft(self, deg_list, act_cnt):
        ...
        ax3.grid(True)
        ax3.plot(range(1,self.MAX_STEP+1), act_cnt)
        ax3.set_title('Action')
        ax3.set_xlabel('step')
        ax3.set_ylabel('action')
        ax3.legend([f'action0:{self.MAX_STEP - sum(act_cnt)}'])
    ...
    def run_test(self, env, MAX_STEP):
        self.MAX_STEP = MAX_STEP
        step = 0
        state = env.reset(1)
        degrees = np.zeros(self.MAX_STEP, dtype=np.float32)
        action_cnt = np.zeros(self.MAX_STEP, dtype=np.int)
        max_angle = 0
        for step in range(self.MAX_STEP):
            ...
            deg = np.rad2deg(th1)
            degrees[step-1] = deg
            max_angle = max(self.max_angle, np.abs(np.rad2deg(th1)))
