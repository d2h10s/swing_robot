...
class a2c_model(tf.keras.Model):
    def __init__(self, observation_n, hidden_n, action_n, load_dir=""):
        ...
        if not self.load_dir:
            ...
            self.max_dir = max(map(lambda s: int(s[s.index('learning_model')+len('leanring_model'):]),glob.glob(os.path.join(self.model_dir, '**'))))
            self.model_dir = os.path.join(self.model_dir, f'learning_model{self.max_dir}')
            self.nn = keras.models.load_model(self.model_dir)
            print(f'model loaded from {self.model_dir}')
        print(self.nn.summary())
        tf.keras.utils.plot_model(self.nn, to_file='./model_img.png', show_shapes=True, show_layer_names=True)
