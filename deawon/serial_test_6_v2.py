...
#model = keras.Model(inputs=inputs, outputs=[actor_mean, actor_std, critic])
model = keras.models.load_model("my_model_3599")
#model = keras.models.load_model("my_final_model_17339")
...
episode_count = 3600
if episode_count != 0:
    with open('reward_history_003599.txt', 'rb') as f:
        reward_list = pickle.load(f)
    running_reward = reward_list[99][2]
    reward_list = []
if episode_count == 0:
    model.save("my_initial_model") 
    ...
    with tf.GradientTape() as tape:
            ...
            if len(buf_rx) == 10:
                if (buf_rx[0] == 0xAA) and (buf_rx[9] == 0xBB):
                    ...
                    if train_state == 0:
                        ...
                        if rad_dot_0_count >= (step_per_second * 80):    
                            rad_dot_0_count = 0
                            episode_reward = 0
                            motor_control = 100
        ...
        if train_state == 2:
            ...
            if running_reward > (max_step * 0.5):  # Condition to consider the task solved
                model.save("my_final_model_" + str(episode_count))
                print("Solved at episode {}!".format(episode_count))

                with open('reward_history_' + str(episode_count % 1000000 // 100000) + str(episode_count % 100000 // 10000) + str(episode_count % 10000 // 1000) +  str(episode_count % 1000 // 100) + str(episode_count % 100) + '.txt', 'wb') as f:
            ...
            if episode_count % 100 == 99:
                model.save("my_model_" + str(episode_count)) 
